...
func main() {
	...
	mux.HandleFunc("/api/v1/visuals", requireAuth(createVisualHandler))
	mux.HandleFunc("/api/v1/visuals/", methodOverride(requireAuthUnlessGet(visualsApiHandler)))
	...
	mux.HandleFunc("/upload/cover", requireAuth(uploadCoverHandler))
	mux.HandleFunc("/upload/portfolio", requireAuth(uploadPortfolioHandler))
	mux.HandleFunc("/upload/visual", requireAuth(uploadVisualHandler))
	mux.HandleFunc("/upload/story", requireAuth(uploadStoryHandler))
	...
	mux.HandleFunc("/api/v1/portfolios", requireAuth(portfolioUploadHandler))
